  ...
  try {
    ...
    try {
      ...
      if (sectRes.ok) {
        ...
        data.author        = parseField(['Written by', 'Author', 'Story', 'Created by', 'Original story', 'Original creator'])
        data.illustrator   = parseField(['Illustrated by', 'Art', 'Artist', 'Art and story', 'Illustrations', 'Drawn by'])
        data.publisher     = parseField(['Published by', 'Publisher', 'English publisher', 'North American publisher'])
        data.serializedIn  = parseField(['Serialized in', 'Magazine', 'Imprint', 'English magazine', 'English serialization'])
        data.originalRun   = parseField(['Original run', 'Published', 'Run'])
        data.volumes       = parseField(['Volumes', 'Volume', 'Original volumes', 'Tankōbon'])
        data.episodes      = parseField(['Episodes', 'Episode', 'No. of episodes', 'Number of episodes'])
        data.directed      = parseField(['Directed by', 'Director', 'Series director'])
        data.studio        = parseField(['Animated by', 'Studio', 'Animation', 'Produced by', 'Production', 'Animation studio'])
        data.genres        = parseField(['Genre', 'Genres', 'Genre(s)'])
          ?.split(/[,;]/)
          .map(g => g.trim())
